...
parse(Tokens,Structure) :-
    ...
    ( contains_syntax_error(Structure) -> throw(Structure)
    ; RemTokens \== [] -> throw('PARSING ERROR(S)'(Tokens,Structure))
    ; true
    ).
...
program(Struct) -->
	...
	epilogue(Es),
	{ Es == [] -> Struct = S
	; Struct = syntax_error(S,epilogue_error(Es))
	}	  
      )
    ; statement_error(L),
      {S=syntax_error('Keyword "begin" is missing.',L)}
    ).
	
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

block(Struct,Vs) -->
    declaration_part(Vs,Ws,Es), statement_list(Ss,Ws),
    { Es==[] -> Struct = Ss
    ; Struct=syntax_error(Es,Ss)
    }.

declaration_part(Vs,Ws,Es) --> [int], !, declarations(Vs,Ws,Es).
